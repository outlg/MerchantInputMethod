...
import com.hit.wi.t9.Business.util.GoodsNamesHttpUtil;
...
public class FunctionViewGroup extends NonScrollViewGroup implements ViewGroupInterface {
    int[] hideAnima = {R.anim.func_key_1_out, R.anim.func_key_2_out, R.anim.func_key_3_out, R.anim.func_key_4_out, R.anim.func_key_5_out, R.anim.func_key_6_out, R.anim.func_key_7_out};
    int[] showAnima = {R.anim.func_key_1_in, R.anim.func_key_2_in, R.anim.func_key_3_in, R.anim.func_key_4_in, R.anim.func_key_5_in, R.anim.func_key_6_in,R.anim.func_key_7_in};
    ...
    public void create(Context context) {
        ...
        viewGroupWrapper.setPadding(0, 12, 0, 0);
        ...
        buttonList.get(point++).setOnTouchListener(companyWordsOnTouchListener);
        buttonList.get(point++).setOnTouchListener(goodsOnTouchListener);
...
//                    lastTouchPoint.y = (int) motionEvent.getRawY();
    ...
    View.OnTouchListener companyWordsOnTouchListener = new View.OnTouchListener() {
        @Override
    ...
    View.OnTouchListener goodsOnTouchListener = new View.OnTouchListener() {
        @Override
        public boolean onTouch(View v, MotionEvent event) {
            if (event.getAction() == MotionEvent.ACTION_UP) {
                if(Final.isLogin){
                    if (Global.isInView(v, event)) {
                        ...
                        GoodsNamesHttpUtil.GetRequest(context, "http://" + MyApplication.ip + ":" + MyApplication.port + "/goods/searchGoods", Final.u_email);
                        while (true) {
                        ...
                        softKeyboard.candidatesViewGroup_goodsName.displayCandidates(Global.SYMBOL, Final.goodsNames, Global.inLarge ? 200 : 9);
                        if (Global.inLarge) softKeyboard.candidatesViewGroup_goodsName.largeTheCandidate();
                        //softKeyboard.refreshDisplay_Goods1(true);
                        softKeyboard.refreshDisplay_GoodsName(true);
                        softKeyboard.candidatesViewGroup_goodsName.largeTheCandidate();
                        Global.inLarge = true;
                        softKeyboard.bottomBarViewGroup.intoReturnState();

                    }
                }else {
                    Toast.makeText(softKeyboard, "登录后方可使用商品功能", Toast.LENGTH_SHORT).show();
                }
            }
