...
public class CandidatesViewGroup extends ScrolledViewGroup {
     ...
     */
    private final int MIN_LAYER_SHOWNUM = 1;
    /**
...
//        SharedPreferences sp = SharedPreferenceManager.getDefaultSharedPreferences(context);
     ...
     */
    private QuickButton addButtonQ(String text, QuickButton button, Boolean isLast) {
        ...
        if ((button.itsLayoutParams.width > remainLength) || isLast) {
            if (remainLength > 20) {
    ...
    public void setCandidates(List<String> words) {
        ...
        for (; i < buttonList.size() && i < words.size(); i++) {
            if (i != words.size() - 1){
                addButtonQ(words.get(i), buttonList.get(i), false);
            }else{
                addButtonQ(words.get(i), buttonList.get(i), true);
            }
        }
