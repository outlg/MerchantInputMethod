...
import com.hit.wi.t9.Business.util.GoodsInfoHttpUtil;
 ...
 * author purebluesong
 */
public class CandidatesViewGroup_goodsName extends ScrolledViewGroup {
     ...
     */
    private final int MIN_LAYER_SHOWNUM = 8;
    /**
    ...
    public CandidatesViewGroup_goodsName() {
...
//        SharedPreferences sp = SharedPreferenceManager.getDefaultSharedPreferences(context);
    ...
    public void displayCandidates(String type, List<String> strings, int show_num) {
        ...
        if (!strings.equals(Collections.EMPTY_LIST)) {
            words = strings.subList(0, Math.min(show_num, strings.size()));
            if(words.size() < MIN_LAYER_SHOWNUM){
                for (int i = words.size(); i < MIN_LAYER_SHOWNUM; i++){
                    words.add("");
                }
            }
        } else {
            for (int i = 0; i < MIN_LAYER_SHOWNUM; i++){
                words.add("");
            }
        }
        setCandidates(words);
    ...
    private QuickButton initNewButton(String text) {
        ...
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
                measureButtonLengthByText(text), ViewGroup.LayoutParams.WRAP_CONTENT
        );
    ...
    private QuickButton addButtonQ(String text, QuickButton button) {
        ...
        getLayerRemainLength(layer);
        button.setText(text);
        button.itsLayoutParams.width = CAND_DIV_NUM * standardButtonWidth;

        layer = getWorkingLayer(layerPointer);
        layerPointer++;
        layer.addView(button, button.itsLayoutParams);
        button.setGravity(Gravity.LEFT |Gravity.CENTER_VERTICAL);
        button.setPadding(20, 10, 10, 20);
    ...
    protected boolean onCandidateTouchEvent(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                Kernel.cleanKernel();
                CharSequence text = ((TextView) v).getText();
                String goodsName = text.toString();
                getInfo(goodsName);
                break;
        }
        return true;
    ...
    public void getInfo(String goodsName){
        if(!goodsName.equals("")){
            /*获取商品名称、商品描述、商品价格、商品Base64字符串*/
            Final.goods.clear();
            ImageUtils.isFinishedGetGoods = false;
            GoodsInfoHttpUtil.GetRequest(context, "http://" + MyApplication.ip + ":" + MyApplication.port + "/goods/searchGoods", Final.u_email, goodsName);
            while (true) {
            ...
            softKeyboard.candidatesViewGroup_goodsInfo.displayCandidates(Final.goods);
            softKeyboard.bottomBarViewGroup.wordsType = 0;
            softKeyboard.refreshDisplay_GoodsInfo(true);
            softKeyboard.candidatesViewGroup_goodsInfo.largeTheCandidate();
    ...
    private OnTouchListener emptyCandidateOnTouch = new OnTouchListener() {
        public boolean onTouch(View v, MotionEvent event) {
            return true;
        }
    };
