...
public class UserWordsAdapter extends RecyclerView.Adapter<UserWordsAdapter.ViewHolder>{
    private List<UserWords> userWordsList;        //一列话术
    Context context;

    //构造函数
    public UserWordsAdapter(Context context, List<UserWords> userWordsList) {
        this.userWordsList = userWordsList;
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        UserWords userWords = userWordsList.get(position);
        holder.words.setText(userWords.getWords());
        if(position%2==0)
