...
public class UserWordsHttpUtil {//获取话术
    ...
    private static void getWords(String url, String email, int checkState,String w_content) {
        ...
        try {
            jsonObject.put("u_email", email);   //获取个人的话术
            if(!w_content.equals(""))
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException{
                try {
                    if (response.body().contentLength() != 0) {
                        ...
                        Final.Userwords.clear();
                        for (int i = 0; i < myResult.length(); i++) {

                            String words_content=myResult.getJSONObject(i).get("w_content").toString();


                            UserWords userWords =new UserWords(words_content);

                            Final.Userwords.add(userWords);
                        }
                    }
                    Final.isFinishedGetWords = true;
