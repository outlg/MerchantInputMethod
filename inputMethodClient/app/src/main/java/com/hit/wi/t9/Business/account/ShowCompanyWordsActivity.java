...
import com.hit.wi.t9.Business.adapter.CompanyWordsAdapter;
import com.hit.wi.t9.Business.util.CompanyWordsHttpUtil;
...
public class ShowCompanyWordsActivity extends BaseActivity{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!Final.isLogin) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(ShowCompanyWordsActivity.this);
            builder.setTitle("警告").setMessage("请先登录")
                    .setNegativeButton("点击登录", new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            Final.isLogin = false;
                            //    Toast.makeText(AddWords_Judge.this, "请先登录", Toast.LENGTH_SHORT).show();
                            startActivity(new Intent(ShowCompanyWordsActivity.this, LoginActivity.class));
                        }
                    }).setPositiveButton("取消", new DialogInterface.OnClickListener() {
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    //Toast.makeText(AddWords_Judge.this, "未登录无法添加个人话术", Toast.LENGTH_SHORT).show();
                    startActivity(new Intent(ShowCompanyWordsActivity.this, WIT9Activity.class));
                }
            }).show();
        } else {
            //连接接口，获取数据
            Final.isFinishedGetWords = false;
            CompanyWordsHttpUtil.GetRequest(ShowCompanyWordsActivity.this, "http://" + MyApplication.ip + ":" + MyApplication.port + "/words/searchWords", Final.u_email, 1,"");
            while (true) {
            ...
            //使用线性布局
            LinearLayoutManager layoutManager = new LinearLayoutManager(ShowCompanyWordsActivity.this);
            imageRecyclerView.setLayoutManager(layoutManager);

            //初始化适配器
            CompanyWordsAdapter adapter = new CompanyWordsAdapter(ShowCompanyWordsActivity.this, Final.Companywords);
            ...
            userwords_search_btn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ...
                    Final.isFinishedGetWords = false;
                    canRequest=false;
                    CompanyWordsHttpUtil.GetRequest(ShowCompanyWordsActivity.this, "http://" + MyApplication.ip +
                            ":" + MyApplication.port + "/words/searchWords", Final.u_email, 1, str);
                    while (true) {
                        if (Final.isFinishedGetWords)
                            break;
                    }

                    if(Final.Companywords.size()==0){
                        Toast.makeText(ShowCompanyWordsActivity.this, "没有找到相关话术", Toast.LENGTH_SHORT).show();
                    }
                    //在这里进行UI操作，将结果显示到界面上
                    imageRecyclerView.setAdapter(adapter);
                    canRequest=true;
...
//                                ShowCompanyWordsActivity.this.runOnUiThread(new Runnable() {
//                                    @Override
//                                    public void run() {
//                                        if(Final.Companywords.size()==0){
//                                            Toast.makeText(ShowCompanyWordsActivity.this, "没有找到相关话术", Toast.LENGTH_SHORT).show();
